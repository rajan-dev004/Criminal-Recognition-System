...
with mode[0]:
    ...
    if run:
        if st.session_state.target_embedding is None:
            st.warning("Please upload a target image containing a face above to enable surveillance.")
        else:
            # If running on Cloud (Hugging Face Spaces), use the st.camera_input widget
            if IS_CLOUD:
                st.info("☁️ Running on Hugging Face Spaces. Using native browser snapshot mode.")
                camera_img = st.camera_input("Scan for Target Criminal")
                ...
                if camera_img is not None:
                    ...
                    if len(faces) == 0:
                        st.info("No faces detected in the snapshot.")
                    else:
                        ...
                        for face_img, (x1, y1, x2, y2) in faces:
                            ...
                            # Compare this face embedding against the target embedding
            ...
            # If running locally, run the live webcam stream loop
            else:
